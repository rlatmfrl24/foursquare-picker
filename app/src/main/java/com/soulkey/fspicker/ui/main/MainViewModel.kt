...
class MainViewModel(
    private val recommendedVenueRepository: RecommendedVenueRepository,
    ...
    fun getRecentLL(): Pair<Float, Float> {
        val recentLocationLatitude =
            recentLocationPreferences.getFloat("latitude", DEFAULT_LATITUDE.toFloat())
        val recentLocationLongitude =
            recentLocationPreferences.getFloat("longitude", DEFAULT_LONGITUDE.toFloat())
    ...
    fun updateCurrentLL(latitude: Double, longitude: Double): Disposable {
        ...
        return recommendedVenueRepository.requestRecommendVenus(latitude, longitude)
            .subscribe(
                { body ->
                    if (body.meta.code == "200") {
                        ...
                        if (body.meta.errorDetail != null) {
                            Toast.makeText(context, body.meta.errorDetail, Toast.LENGTH_SHORT)
                                .show()
                        }
                    }
